...
int main()
{
  ...
  h.onMessage([&ukf,&target_x,&target_y,&target_v,&target_yaw,&target_dyaw](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
		  ...
		  if (target_dyaw == 0)
		  {
			  target_dyaw = ukf.x_[4];
		  }
		  else
		  {
			  target_dyaw = (19 * target_dyaw + ukf.x_[4]) / 20;
		  }
		  ...
		  // Predict where the vehicle is going to be in delta_t time
		  double delta_t = 1.5;
		  for (size_t i = 1; i < 11; i++)
		  {
			  target_x = target_x + target_v*(delta_t/10)*cos(target_yaw);
			  target_y = target_y + target_v*(delta_t/10)*sin(target_yaw);
			  target_yaw = target_yaw + (target_dyaw*(delta_t/10));
		  }
		  
		  // Calculate steering angle
    	  double heading_to_target = atan2(target_y - hunter_y, target_x - hunter_x);
		  int coef = heading_to_target / (2.*M_PI);
		  heading_to_target = heading_to_target - coef*2.*M_PI;
		  ...
		  cout << "Diff:" << heading_difference << '\n ';
		  cout << "Target:" << heading_to_target << '\n ';
		  cout << "Current:" << hunter_heading << '\n ';
          json msgJson;
          msgJson["turn"] = max(min(0.025, heading_difference),-0.025);
          msgJson["dist"] = (distance_difference*distance_difference/150);
